...
def predicted_to_intervals(pred_classes, frame_sec, frame_shift):
    start = -1
    last = -1
    current_time = 0
    intervals = []
...
def main():
    ...
    pred_classes = predictor.predict(np.expand_dims(feature_df, axis=0))[0]
    intervals = predicted_to_intervals(pred_classes, frame_sec=0.5, frame_shift=0.5/4)
