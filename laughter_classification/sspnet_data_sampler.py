...
class SSPNetDataSampler:
    ...
    def _interval_generator(incidents):
        for itype, start, end in chunks(incidents, 3, 3):
            if itype == 'laughter':
                yield start, end
    ...
    def get_labels_for_file(self, wav_path, frame_sec):
        ...
        is_laughter = np.array([self.most(la) for la in chunks(laught_along, frame_size, frame_size // 4)])
    ...
    def df_from_file(self, wav_path, frame_sec):
        ...
        """
        features = LibrosaExtractor(frame_sec).extract_features(wav_path)
